...
sched_yield(void) {
    ...
    size_t lastenv_no, nextenv_no;

    lastenv_no = curenv ? curenv - envs : 0;
    nextenv_no = (lastenv_no + 1) % NENV;

    while (nextenv_no != lastenv_no) {
        if (envs[nextenv_no].env_status == ENV_RUNNABLE ||
            envs[nextenv_no].env_status == ENV_RUNNING) {
            break;
        }

        nextenv_no = (nextenv_no + 1) % NENV;
    }

    if (envs[nextenv_no].env_status == ENV_RUNNABLE ||
        envs[nextenv_no].env_status == ENV_RUNNING) {
        env_run(&envs[nextenv_no]);
    }
