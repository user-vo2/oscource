...
static int
sys_fb_putc(uint32_t fb_addr, uint32_t color) {
    /* Check that the user has permission to read memory [s, s+len).
     * Destroy the environment if not. */
    // cprintf("%x %x\n", fb_addr, color);
    if (color != 0xff000000 && fb_addr != 0xffffffff) {
        framebuff[fb_addr] = color;
        // cprintf("2\n");
    } else {
        cprintf("3\n");
        display_ready_picture_bmp(framebuff);
    }
...
syscall(uintptr_t syscallno, uintptr_t a1, uintptr_t a2, uintptr_t a3, uintptr_t a4, uintptr_t a5, uintptr_t a6) {
    ...
    case SYS_fb_putc:
        // cprintf("1\n");
        return sys_fb_putc((uint32_t)a1, (uint32_t)a2);
    default:
        return -E_NO_SYS;
    }
    // LAB 11: Your code here
