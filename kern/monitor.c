...
int mon_intelman(int argc, char **argv, struct Trapframe *tf);
...
static struct Command commands[] = {
        ...
        {"intelman","Displays Intel Software Developer Manual link", mon_intelman},
...
mon_backtrace(int argc, char **argv, struct Trapframe *tf) {
    ...
    for (uint64_t rbp = read_rbp(); rbp; rbp = *((uint64_t*)rbp)) {
        cprintf("  rbp = %016lx", rbp);
        uint64_t rip = ((uint64_t*) rbp)[1];
        cprintf("  rip = %016lx\n", rip);
        struct Ripdebuginfo info;
        debuginfo_rip(rip, &info);
        cprintf("    %s:%d: %s+%lu\n", info.rip_file, info.rip_line, info.rip_fn_name, rip - info.rip_fn_namelen);
    }
    return 0;
}

int
mon_intelman(int argc, char **argv, struct Trapframe *tf) {
    cprintf("https://software.intel.com/content/www/us/en/develop/articles/intel-sdm.html#combined\n");
    return 0;
}
